...
- **Problem Set 2** is due 10/23
- Review questions in Problem Set 1
...
library(pacman)
p_load(tidyverse, psych, gridExtra, foreign, 
       stargazer, ggcorrplot, kableExtra)
...
1. Simulate the data with the same relationship as specified above, only **changing the residual term from having a sd = 10 to sd = 50**. Follow the steps demonstrated above, create a dataframe and fit an OLS model to your simulated data. Create a scatterplot of your data. *Note:* Remember to `set.seed()`! 

2. Then, using the formula we have learned in the lecture, hand-code the following statistics:  
  (a) The value of TSS
  (b) $R^2$
...
```{r part1-exercise-Q2}
## TSS
TSS <- sum((earn2 - mean(earn2))^2)

## SSE
SSE <- sum((earn2 - m_simu_exercise$fitted.values)^2)
...
## SE of beta1
MSE <- SSE/(length(edu2)-2)
se <- sqrt(
  MSE/sum((edu2 - mean(edu2))^2)
)

## 95% CI
upper <- summary(m_simu_exercise)$coefficients[2,1] + 1.96*se
lower <- summary(m_simu_exercise)$coefficients[2,1] - 1.96*se
```
...
# Combine dataframe
df_exercise <- data.frame(x_edu = edu2, y_earn = earn2)
...
Once we have simulated the sample data, we can see how different model setup performs.  
...
$$
Y = \underbrace{10 + 3 \cdot \bar{inc}}_{Intercept} + 5 \cdot edu +  \epsilon
$$
...
df %>%
  ...
  theme_bw() +
  labs(title = "Education and Y by income levels with fitted OLS line (with interaction)")
...
Let us look at the graphs on the effects of education on Y when there is an interaction effect between education and income:
