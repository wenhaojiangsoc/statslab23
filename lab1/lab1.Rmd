...
* Email me any time if you have questions about coding or environmental setup. 
...
* For Mac users, download and install [MacTex](https://tug.org/mactex/). For Windows users, download and install [MiKTeX](https://miktex.org/download). We will need a TeX installation to knit R Markdown codes into PDFs to complete assignments.
...
### Coding in R Script v.s. R Markdown
...
+ If you come across an error message, debug your code by:
  ...
  (ii) Googling your error massage. Nowadays you can also ask ChatGPT.
...
+ You don't have to learn coding and formating LaTeX from scratch. There are a wide variety of templates ready to use on Overleaf. For example, Overleaf provides the template for [PNAS](https://www.overleaf.com/gallery/tagged/pnas).
        
### What is Overleaf

+ Overleaf is a collaborative cloud-based **LaTeX editor**.  

+ You can check out a demo document on Overleaf [here](https://www.overleaf.com/read/rxpsgfgjczbk).

+ Overleaf offers comprehensive guidance on how to create and edit LaTeX documents.  

+ For the final project, you will writing paragraphs, creating tables and figures, etc. using LaTeX. I will provide you with a basic template with instructions. You can also search and learn the specifics when you come across questions or problems -- such as how to insert tables, how to fit tables to page, insert equations, how to change the font size, how to organize citations, etc.  

+ I strongly recommend taking a look at [the 30-minute quick guide](https://www.overleaf.com/learn/latex/Learn_LaTeX_in_30_minutes) on Overleaf.
...
+ To learn `tidyverse` and coding in R more systematically, you may check the free online book by Wickham & Grolemund (2017): [R for Data Science](https://r4ds.had.co.nz/index.html). (A lot of the examples used in this lab is drawn from the book.)
...
+ You can find the path of the folder by right-clicking it and `get info` (on Mac).


```{r import csv}
# set working directory
setwd("/Users/wenhao/Library/CloudStorage/Dropbox/Teaching/Graduate Stats github/lab1/data")

# Load csv files
gapminder <- read.csv("gapminder.csv") 
tidy_df1 <- read.csv("tidy_example_1.csv")
tidy_df2 <- read.csv("tidy_example_2.csv")
```
...
+ Common things to check about your data: 
  ...
  + Alternatively, I am more used to `dim(data_object)`, which gives both the number of observations and the number of variables.
...
# Check mean, variance, sd, sum, length of a variable (column) (related to your HW)
...
```{r rename, eval = F}
# ---------- Rename ---------- 
rename(gapminder, population = pop) # new name = old name
...
+ You can also do the above using one single function, which can also be piped to your data object
...
## Part 6 Preparations for Assignment 1
...
## this is to ensure you can replicate results, even if population creation and sampling is "randomized"
set.seed(2023)
