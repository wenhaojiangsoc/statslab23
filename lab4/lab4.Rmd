...
- **Problem Set 1** is due on Tue. Sep. 29th, 11:59 pm. Remember to submit both your coding file (the R Markdown) and a PDF file knitted from the R Markdown
...
$$SSE = \sum^{n}_{i=1}\epsilon^2 = \sum^{n}_{i=1} \left(y_i - (\hat\beta_0 + \hat\beta_1 x_i) \right) ^2 $$
...
> **Prerequisite (IMPORTANT!)**: Before starting this exercise, make sure you are working in a devoted lab4 folder that has a `.Rproj` file in place. Open the lab4.Rmd under the `lab4.Rproj` project.
...
<p align="center">
![](graph/codebook_demo2.png){width=60%}
</p>  
...
3. Use `lm()` to fit a OLS regression model with INCWAGE as the dependent variable and EDUC as the independent variable. Report your $\hat{\beta_0}$, $\hat{\beta_1}$, and SSE. No need to weight individuals at this point.
